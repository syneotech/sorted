    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white p-4 rounded-lg shadow-sm">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-sm text-gray-600">Show matched items only</span>
        </label>
...
function MenuItemRow({ item }: { item: ComparisonMenuItem }) {
  ...
  const zomatoOnly = !item.swiggy && item.zomato;
      ...
      <div className="text-right shrink-0">
        ...
        {isMatched && cheaper && cheaper !== 'same' && item.priceDifference && (
          <p className="text-xs text-green-600 mt-1">
            Save ₹{item.priceDifference.savings} on {cheaper === 'swiggy' ? 'Swiggy' : 'Zomato'}
          </p>
        )}
      </div>
    </div>
